...
var WRITEHERE_BOUND_PRESETS = ["article-editor", "xieka"];
...
// packages/writehere/src/prepare-hook.ts
import { emitAgentEvent } from "@deepseek-ai/dsh-agent";
var boundPresets = new Set(WRITEHERE_BOUND_PRESETS);
...
function hostLooksUpDrivers(prepare) {
  return Function.prototype.toString.call(prepare).includes("agentDrivers");
}
function exclusiveWriteHerePreset(preset) {
  return preset !== void 0 && boundPresets.has(preset);
}
function prepareWithCtor(loop, Ctor, ownerCtx, id, options, session, callerSignal) {
  ...
  const onFactoryTeardown = () => {
    abort.abort(loop.ownership.signal.reason);
  };
...
function hookAgentLoopPrepare(ctx) {
  const loop = ctx.get("agentLoop");
  if (loop === void 0 || typeof loop.prepare !== "function") return;
  const orig = loop.prepare;
  if (hostLooksUpDrivers(orig)) return;
  const hooked = function(ownerCtx, id, options, session, callerSignal) {
